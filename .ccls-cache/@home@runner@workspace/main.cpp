...
std::cout << "\nSum of scores: " << sum << std::endl;

//Calculate and display average
double average = getAverage(sum, size);
std::cout << "Average of scores: " << average << std::endl;
std::cout << std::fixed << std::setprecision(2) << average << std::endl;
std::cout << "Average of scores: " << std::fixed << std::setprecision(2) << average << std::endl;

//  Add bonus to scores
addBonus(scores, size, 5); //Default bonys of 5 per instrucions
...
//Count and display number of students who passed
int passed = countPassed(scores, size);
std::cout << "Number of students who passed (>=70): " << passed << std::endl;

return 0;
} // <--- Main closes here 

int calcualteSum(const int arr[], int size)
int sum = 0;
for (int i = 0; i < size; i++){
    sum += arr[i];
    return sum;
